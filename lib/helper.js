...
const getTreePost = (tags, tagNames) => {
  const rootTagNames = tagNames || tags.map((v) => v.name);
  ...
  return tags.filter((tag) => {
    ...
    const childTagNames = rootTag.posts.reduce(
      (tags, post) => {
...
function listRootTagHelper(tags, options) {
  ...
  let result = '';
  const self = this;
  const rootTagOption = options.hasOwnProperty('root_tag_option') ? Object.assign(defaultOption, options.root_tag_option) : defaultOption;
  const tagNames = rootTagOption.root_tag_names;
  const subTagLimit = rootTagOption.sub_tag_limit;
  // rootTags
  const treeTags = getTreePost(tags, tagNames);
  // eslint-disable-next-line node/no-unsupported-features
...
exports.getTreePost = getTreePost;
